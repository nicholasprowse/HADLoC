...
def encode_labels(parser):
    ...
    Args:
        parser: The parser containing the instructions to rewrite
    """
    instructions, labels = parser.instructions, parser.labels
    finished = {}

    for label in labels:
        labels[label] = {'orig': labels[label], 'min': labels[label], 'max': labels[label], 'len': 0}
...
def find_lengths(labels, finished, instructions):
    ...
    Args:
        labels: Dictionary containing the relevant information for all the labels
...
def find_deterministic_lengths(labels, finished, instructions):
    """
    ...
    changed = True
    while changed:
        update_minmax(labels, instructions)
...
def update_minmax(labels, instructions):
    """
            ...
            if type(instruction[1]) == String:
                if labels[instruction[1]]['len'] == 0:
                    for label, pos in labels.items():
                        if pos['orig'] > i:
                            pos['max'] += 1
                elif labels[instruction[1]]['len'] == 2:
                    for label, pos in labels.items():
                        if pos['orig'] > i:
                            pos['min'] += 1
                            pos['max'] += 1
