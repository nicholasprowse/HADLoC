...
class Parser:
    ...
    def class_declaration(self):
        ...
        **ClassBody**         ::=  '{' (FieldDeclaration | MethodDeclaration | Constructor)* '}'
        ...
        self.add_privacy_modifier(node)
    ...
    def method_declaration(self):
        ...
        """
        node = ASTNode('MethodDeclaration', {'Statements': [], 'ParameterNames': [], 'ParameterTypes': []})
        self.add_privacy_modifier(node)

        if self.match('static'):
            node.addchild('Static', self.value(-1))
        else:
            node.addchild('Static', None)
        ...
        if self.parse(self.type, node, 'ParameterTypes'):
            if not self.get_type() == Token.identifier:
                raise CompilerException(CompilerException.SYNTAX,
                                        "Expected parameter identifier after type in method parameter list",
                                        self.value(-1))
            node.addchild('ParameterNames', self.value())
            self.index += 1

        while self.match(','):
            if not self.parse(self.type, node, 'ParameterTypes'):
                raise CompilerException(CompilerException.SYNTAX,
                                        "Expected method parameter after comma", self.value(-1))
            if not self.get_type() == Token.identifier:
                raise CompilerException(CompilerException.SYNTAX,
                                        "Expected parameter identifier after type in method parameter list",
                                        self.value(-1))
    ...
    def constructor_declaration(self):
        ...
        """
        node = ASTNode('ConstructorDeclaration', {'Statements': [], 'ParameterNames': [], 'ParameterTypes': []})
        self.add_privacy_modifier(node)
    ...
    def field_declaration(self):
        ...
        Creates a node with the following children
            ...
            - VariableInitializers - A list of ASTNodes containing VariableInitializer nodes. If an initializer is
              omitted then the relevant entry in this list will be None
    ...
    def add_privacy_modifier(self, node):
        """
        Checks if the current token is a privacy modifier (one of 'private', 'public' or 'protected'). If it is, then
        token is added to the given node under the name PrivacyModifier. Otherwise, None is added under that name

        Args:
            node (ASTNode): The node to add the privacy modifier to
        """
        if self.match('private') or self.match('public') or self.match('protected'):
        ...
        for name, child in self.children.items():
            ...
            if type(child) is list:
                for i in range(len(child)):
                    if type(child[i]) is ASTNode:
                        print('|   ' + prefix + str(i))
                        child[i].print_tree(depth + 2)
                    else:
                        print('|   ' + prefix + str(child[i]))
