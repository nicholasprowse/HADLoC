...
class Parser:
    ...
    def class_declaration(self):
        ...
        **ClassBody**         ::=  '{' (FieldDeclaration | MethodDeclaration | Constructor)* '}'
        ...
        while self.parse(self.field_declaration, node, 'Fields') or \
                ...
                self.parse(self.constructor_declaration, node, 'constructors'):
            pass
    ...
    def non_function_type(self):
        ...
        if self.match('('):
            ...
            if not self.match(')'):
                raise CompilerException(CompilerException.SYNTAX, "Unmatched bracket. Expected closing bracket after "
                                                                  "function type definition", self.tokens[start])
        ...
        while self.match('['):
            ...
            if not self.match(']'):
                raise CompilerException(CompilerException.SYNTAX,
                                        "Expected closing square bracket, '[', after the opening square bracket, ']', "
                                        "in array type", self.value(-1))
            node.addchild('Dims', self.value(-1))
